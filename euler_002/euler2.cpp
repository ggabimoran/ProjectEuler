...
Il est facile de montrer que les termes à valeur pair sont les F_{3n+2} pour tout n. On peut par ailleurs vérifier par récurrence que pour tout n F_{3n+2} = F_{3n-1} + F_{3n-4}, donc F_{3n+2} est une suite récurrente linéaire homogène d'ordre 2 à coefficients constants. On trouve son expression grâce à l'équation caractéristique. En notant r_1 = 2 + sqrt(5) et r_2 = 2 - sqrt(5), on a : 
...
int main(){
  clock_t begin = clock();
  int N=(int)(log(4000000/(1+2/sqrt(5)))/log(2+sqrt(5)));
  double r_1 = 2 + sqrt(5);
  double r_2 = 2 - sqrt(5);
  int sum = (1+2/sqrt(5))*(1-pow(r_1,N+1))/(1-r_1)+(1-2/sqrt(5))*(1-pow(r_2,N+1))/(1-r_2);
  clock_t end = clock();
